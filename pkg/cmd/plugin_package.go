...
func (o *pluginPackageOptions) run(out io.Writer) error {
	...
	// Validate it's a valid plugin directory
	if _, err := plugin.LoadDir(o.pluginPath); err != nil {
	...
	pluginName := filepath.Base(o.pluginPath)
	tarballPath := filepath.Join(o.destination, pluginName+".tgz")
	...
	if err := plugin.CreatePluginTarball(o.pluginPath, tarFile); err != nil {
